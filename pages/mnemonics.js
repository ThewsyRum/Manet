...
import PINCODE from "./PassWord";


LogBox.ignoreLogs([
  'Non-serializable values were found in the navigation state',
]);
class Mnemonics extends Component {
    constructor(props){
      super(props);
      this.state = {
  ...
  Mnemonics= async()=>{
    ...
    try {
        ...
        if (value !== null) {
            ...
            this.setState({mnemo:value,mnemnoics:value.split(' ')})
        }else{
            console.log("Empty")
        }
    } catch (error) {
...
render() {
  const {mnemo,pincorrect,mnemnoics} = this.state
  return (
    !pincorrect?
...
<Pressable onPress={()=>{this.props.navigation.goBack()}}
                                    style={{marginTop:"20%",left:30,position:"absolute",borderRadius:30}}
                                    >
                    <Icon name="chevron-left" style={{fontSize:25}}></Icon>
                </Pressable>

        <View style={styles.iconRow}>
          <Icon name="plus" style={styles.icon}></Icon>
          <Text style={styles.newWallet}>Your Mnemomics</Text>
        </View>
        ...
        <TouchableOpacity
          onPress={()=>{ Clipboard.setString(mnemo);
            alert('Copied')}}
          style={styles.button}
        >
